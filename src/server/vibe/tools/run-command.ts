...
import { RUN_COMMAND_DESCRIPTION } from "./descriptions";
	...
	tool({
		...
		inputSchema: z.object({
			...
			command: z
				.string()
				.describe(
					"The base command to run (e.g., 'npm', 'node', 'python', 'ls', 'cat'). Do NOT include arguments here. IMPORTANT: Each command runs independently as a fresh process - there is no persistent state between commands. You cannot use 'cd' to change directories for subsequent commands.",
				),
			args: z
				.array(z.string())
				.optional()
				.describe(
					"Array of arguments for the command. Each argument should be a separate string (e.g., ['install', '--verbose'] for npm install --verbose, or ['src/index.js'] to run a file, or ['-la', './src'] to list files). IMPORTANT: Use relative paths (e.g., 'src/file.js') or absolute paths instead of trying to change directories with 'cd' first, since each command runs as an independent process.",
				),
			sudo: z
				.boolean()
				.optional()
				.describe("Whether to run the command with sudo (not supported, ignored)"),
			wait: z
				.boolean()
				.describe(
		...
		execute: async (
			...
			if (!wait) {
				...
				return `The command \`${command} ${args.join(" ")}\` has been started in the background in the sandbox with ID \`${sandboxId}\` with the commandId ${cmd.cmdId}.`;
			}
