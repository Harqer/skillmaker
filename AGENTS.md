...
# System Architecture & Component Roles Reference

Refer to `/ARCHITECTURE_REFERENCE.md` for full details. Quick summary of system roles:
1. **Firecrawl / Scraper (`agent/scraper.py`):** Ingests raw documentation from URLs into clean Markdown files and directory trees.
2. **LLM Synthesis (`src/server/ai.ts`):** Converts raw Markdown documentation into structured **EVE Skill Bundles** (`SKILL.md`, `rules/`, `scripts/`, `references/`).
