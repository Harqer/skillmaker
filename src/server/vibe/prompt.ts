...
1. **Create Sandbox**
   ...
   - Initializes a Linux environment that will serve as the workspace for the session.
...
3. **Run Command**
   ...
   - Never combine commands with \`&&\` or assume persistent state; commands must be run sequentially with the \`wait\` flag used for dependencies.
...
- Early stop (act if any): You can name exact files/symbols to change, or you can repro a failing test/lint or have a high-confidence bug locus.
...
export const errorsPrompt = `You are an expert software engineer reviewing \`stderr\` logs from a development sandbox. Your task is to detect **actionable errors that require code fixes**.
