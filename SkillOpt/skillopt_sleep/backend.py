...
class GeminiBackend(CliBackend):
    """Drives Google Gemini API via REST generateContent endpoint using GEMINI_API_KEY."""

    name = "gemini"

    def __init__(self, model: str = "", api_key: str = "", timeout: int = 180) -> None:
        model_name = model or os.environ.get("GEMINI_MODEL") or "gemini-2.5-flash"
        super().__init__(model=model_name, timeout=timeout)
    ...
    def _call(self, prompt: str, *, max_tokens: int = 1024, retries: int = 3) -> str:
        ...
        if not key:
            self.last_call_error = "GEMINI_API_KEY not found in environment"
            print(f"[gemini_backend] Error: {self.last_call_error}")
            return ""

        model_id = self.model or "gemini-2.5-flash"
                    ...
                    if candidates:
                        ...
                        if parts:
                            ...
                            if text:
                                self.last_call_error = ""
                                return text
                    self.last_call_error = "Empty text candidate in Gemini API response"
            except Exception as exc:
                self.last_call_error = f"Gemini API call failed: {exc}"
                print(f"[gemini_backend] Attempt {attempt + 1} failed: {exc}")
...
def get_backend(
    ...
    *,
    ...
    if n in {"gemini", "google", "gemini_api", "genai"}:
