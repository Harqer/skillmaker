...
def _role_env(role: str, key: str, default: str) -> str:
    ...
    val = os.environ.get(role_key) or os.environ.get(generic_key)
    if val:
        return val
...
def _initial_config(role: str) -> OpenAICompatibleConfig:
    ...
    default_model = "gemini-3.6-flash" if os.environ.get("GEMINI_API_KEY") else default_model_for_backend(BACKEND_NAME)
